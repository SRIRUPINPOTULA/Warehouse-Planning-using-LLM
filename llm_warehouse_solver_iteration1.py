...
inst_excerpt = "\n".join(inst_lines[:120])  # adjust if your inst is larger/smaller
...
STRICT RULES
-----------------------------------
1. Output ONLY valid Clingo code. No markdown. No triple backticks. No explanations. No comments.
...
5. Time must be symbolic (use #const maxT = 20. and time/1), do NOT hardcode specific action times.
...
at(shelf,S,X,Y,0) :- init(object(shelf,S), value(at,pair(X,Y))).
...
TASK
-----------------------------------
Generate the ASP encoding for the Automated Warehouse problem that:
- models robot movement on node/2 (adjacent 4-neighborhood plus wait)
- models pickup, putdown, and delivery
- enforces inertia (frame axioms)
- prevents collisions and swaps (robots, and if needed shelves while carried)
- prevents shelves on highway cells at any time
- ensures all order lines are fulfilled by time maxT
- minimizes makespan using #minimize (earliest time when all orders fulfilled)
...
# -------------------------------
# Clean + Save Step 2 Output
# -------------------------------
domain_text = response.text.strip()

# If the model ever wraps output, strip code fences defensively
domain_text = re.sub(r"^```[a-zA-Z]*\s*", "", domain_text)
domain_text = re.sub(r"\s*```$", "", domain_text)

output_file = "warehouse_domain.lp"
